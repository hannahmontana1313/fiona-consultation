...
export default async function handler(req, res) {
  ...
  const { minutes, tarif, userId, prenom, telephone, domaine, sujet, message, ancienConsultationId, prioritaire, prixFinal, cadeauUtilise, statutVIP } = req.body;

  if (!minutes || !userId) return res.status(400).json({ error: 'Paramètres manquants' });

  // Utiliser le prix final si fourni (avec remise), sinon calculer normalement
  const montantCentimes = prixFinal
    ? Math.round(prixFinal)
    : Math.round((minutes * tarif + (prioritaire ? 7 : 0) + 0.25) / (1 - 0.014) * 100);
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        cadeauUtilise: cadeauUtilise ? String(cadeauUtilise) : '',
